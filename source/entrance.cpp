...
static bool ValidateWorld(Entrance* entrancePlaced) {
    ...
    bool checkPoeCollectorAccess =
        ...
        (entrancePlaced == nullptr /*|| Settings::MixedEntrancePools.IsNot(MIXEDENTRANCES_OFF)*/ ||
    ...
    bool checkOtherEntranceAccess =
        (Settings::ShuffleOverworldEntrances ||
         Settings::ShuffleInteriorEntrances.Is(SHUFFLEINTERIORS_ALL) /*|| Settings::ShuffleOverworldSpawns*/) &&
        (entrancePlaced == nullptr /*|| Settings::MixedEntrancePools.IsNot(MIXEDENTRANCES_OFF)*/ ||
    ...
    // This is mostly relevant when mixing entrance pools or shuffling special interiors (such as windmill or kak potion
    // shop) Warp Songs and Overworld Spawns can also end up inside certain indoors so those need to be handled as well
...
int ShuffleAllEntrances() {
    ...
    // delete all targets that we just placed from one way target pools so multiple one way entrances don't use the same
    // target
