...
constexpr std::array ItalianDungeonArticles = {
    "del", "della", "della", "del", "del", "del", "del", "del", "del", "della", "della", "della", "della", "del",
};
...
void CreateAlwaysIncludedMessages() {
    ...
    CreateMessage(
        ...
        INSTANT_TEXT_ON() + "Deutsch" + NEWLINE() + NEWLINE() + TWO_WAY_CHOICE() + COLOR(QM_GREEN) + "Buy" + NEWLINE() +
            "Don't buy" + COLOR(QM_WHITE) + MESSAGE_END());
    ...
    CreateMessage(0x9D9, 0, 2, 3,
                  UNSKIPPABLE() + ITEM_OBTAINED(ITEM_KEY_BOSS) + INSTANT_TEXT_ON() + "You got the " +
                      COLOR(DungeonColors[DUNGEON_INSIDE_GANONS_CASTLE]) +
                      EnglishDungeonNames[DUNGEON_INSIDE_GANONS_CASTLE] + NEWLINE() + "Boss Key" + COLOR(QM_WHITE) +
                      "!" + INSTANT_TEXT_OFF() + MESSAGE_END(),
                  UNSKIPPABLE() + ITEM_OBTAINED(ITEM_KEY_BOSS) + INSTANT_TEXT_ON() + "Vous trouvez la " +
    ...
    for (u32 smallKey = 0; smallKey <= (DUNGEON_BOTTOM_OF_THE_WELL - DUNGEON_FOREST_TEMPLE); smallKey++) {
        ...
        CreateMessage(0x9300 + smallKey, 0, 2, 3,
                      ...
                      UNSKIPPABLE() + ITEM_OBTAINED(ITEM_KEY_SMALL) + INSTANT_TEXT_ON() + "Hai ottenuto il " +
                          COLOR(DungeonColors[dungeon]) + "mazzo di chiavi " + ItalianDungeonArticles[dungeon] +
                          NEWLINE() + ItalianDungeonNames[dungeon] + COLOR(QM_WHITE) + "!" + INSTANT_TEXT_OFF() +
                          MESSAGE_END(),
                      UNSKIPPABLE() + "Deutsch" + MESSAGE_END());
    }
    for (u32 smallKey = 0; smallKey <= (DUNGEON_INSIDE_GANONS_CASTLE - DUNGEON_GERUDO_TRAINING_GROUNDS); smallKey++) {
    ...
    CreateMessage(0x70F8, 0, 0, 0,
                  ...
                  UNSKIPPABLE() + "Aspetta un attimo, bello mio!" + WAIT_FOR_INPUT() + NEWLINE() + UNSKIPPABLE() +
                      ...
                      WAIT_FOR_INPUT() + NEWLINE() + UNSKIPPABLE() + "Grazie a te, ora ho una bella scorta" +
                      NEWLINE() + "di Grandi Poo e non devo comprare" + WAIT_FOR_INPUT() + NEWLINE() + UNSKIPPABLE() +
    ...
    CreateMessage(0x9002, 0, 2, 3,
                  ...
                  UNSKIPPABLE() + INSTANT_TEXT_ON() + CENTER_TEXT() + COLOR(QM_RED) + "STOLTO!" + COLOR(QM_WHITE) +
                      INSTANT_TEXT_OFF() + MESSAGE_END(),
                  UNSKIPPABLE() + INSTANT_TEXT_ON() + CENTER_TEXT() + COLOR(QM_RED) + "Deutsch" + COLOR(QM_WHITE) +
                      INSTANT_TEXT_OFF() + MESSAGE_END());
    // Curse Traps
    ...
    for (u32 price = 0; price <= 95; price += 5) {
        CreateMessage(
            ...
            INSTANT_TEXT_ON() + "Deutsch" + NEWLINE() + NEWLINE() + TWO_WAY_CHOICE() + COLOR(QM_GREEN) + "Y" +
